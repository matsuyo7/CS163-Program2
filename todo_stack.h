...
struct node
{
	binder * collection;	//a dynamically allocated array of binders
...
class stack
{
	public:
		stack();	//constructor
		~stack();	//destructor
		int push(const binder & to_add);	//takes a binder as an argument to add the binder to the top of the stack. return success/failure
